...
impl From<JumpSequence> for String {
    fn from(mut seq: JumpSequence) -> Self {
        seq.0.reverse();
        String::from_utf8(seq.0).expect("Jump keys should be UTF-8")
    }
}
...
pub trait JumpSequencer {
    ...
    fn try_get_pos(&self) -> Option<usize>;
}
...
impl From<u8> for TrieNode {
    fn from(key: u8) -> Self {
        TrieNode {
            key,
            children: vec![],
            pos_in_text: None, // Calculation happens after trie construction
        }
    }
}
...
fn attach_jump_targets_to_leaves(
    node: &mut TrieNode,
    jump_targets: &mut impl Iterator<Item = usize>,
) {
    if node.children.is_empty() {
        node.pos_in_text = jump_targets.next();
        return;
    }
...
impl TrieNode {
    pub fn build(keys: &[u8], jump_targets: Vec<usize>) -> Self {
...
fn depth_first_search(node: &TrieNode) -> Vec<(usize, JumpSequence)> {
    let key = node.key;
    if node.children.is_empty() {
        return vec![(node.pos_in_text.unwrap(), JumpSequence::from(key))];
    }
    node.children
        .iter()
        .flat_map(|child| {
...
impl JumpSequencer for TrieNode {
    fn generate(&self) -> Vec<JumpAnnotation> {
        ...
        self.children
            ...
            .map(|(loc, seq)| JumpAnnotation {
                loc,
                keys: String::from(seq),
            })
            .collect()
    }
...
mod jump_tests {
    ...
    fn more_jump_targets_than_keys_1() {
        let mut paths = TrieNode::build(b"xyz", vec![9, 1, 5, 100])
            .generate()
            .into_iter();
        assert_eq!(next(&mut paths), Some((String::from("x"), 9)));
        assert_eq!(next(&mut paths), Some((String::from("y"), 1)));
